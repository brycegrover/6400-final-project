...
Previous studies have also used routing algorithms and reinforcement learning to improve food-delivery assignment [@giraldo2024; @cheng2025]. However, they pay less attention to how the road network itself affects delivery performance. Even with a good assignment strategy, important roads or intersections may still become bottlenecks. Road closures and high demand can also cause longer delivery times. Based on this, our project studies both the road network and courier assignment strategies.
...
![Free flow drive time from the restaurant core.](outputs/07_courier_reach.png){#fig-reach width=70%}
...
## RQ7: Targeted and Random Road Closures

To address how road closures affect delivery performance, we used the Washington, DC road network and the 15,188 simulated orders. Where the network contains 10,027 nodes, 26,938 directed edges, and 16,267 physical road segments.

First, we calculated the baseline route for each order using Dijkstra's shortest-path algorithm. Then, we closed 1% of the physical road segments, which equals 163 roads. Both directions of each road were removed. For targeted closures, we removed the 163 roads used by the largest number of baseline delivery routes. For random closures, we randomly removed 163 roads and repeated the experiment 30 times. Using the same number of roads makes the comparison fair.

After each closure, we rerouted all orders. Also, we measured the additional travel time for reachable orders and the number of orders that lost a route. Lastly, we compared the NAC and SWB strategies under fleets of 200 and 500 couriers.

## RQ8: Fleet Size

To determine the ideal number of couriers, we used the fleet-sweep simulation results. The tested fleet sizes were 100, 150, 200, 300, 400, 500, 600, and 800 couriers. Also, each fleet size was tested under the two delivery strategies.

For each setting, we first measured the average delivery time, then calculated the percentage delivered within 45 minutes, performance during the 7:00 PM peak, and courier utilization. Also, we defined the ideal fleet as the smallest tested fleet that meets two conditions. First, both strategies must deliver at least 99.5% of orders within 45 minutes. Second, the next tested fleet must save less than one additional minute on average.

Since the gaps between fleet sizes are different, we also calculated the number of minutes saved per 100 additional couriers. This allowed us to measure when adding more couriers stopped providing a large improvement.
...
@tbl-overall-strategy shows the overall delivery performance when both fleet sizes are combined.

| Strategy | Orders | Mean (min) | Median (min) | Standard deviation (min) |
|:---------|-------:|-----------:|-------------:|-------------------------:|
| SWB      | 30,376 | 75.61      | 28.29        | 78.50                    |
| NAC      | 30,376 | 79.78      | 27.56        | 84.84                    |

: Overall delivery time by assignment strategy. {#tbl-overall-strategy}

Moreover, as we can see from @tbl-overall-strategy, when both fleet sizes were combined, SWB had the fastest overall average delivery time. Its average was 75.61 minutes, compared with 79.78 minutes for NAC. Based on these results, we see that the best strategy depends on fleet size. SWB works better when the number of couriers is limited, while NAC is slightly faster when 500 couriers are available.
...
![Added travel time and unreachable orders.](outputs/17_network_closure_damage.png){#fig-closure-damage width=85%}

As we can see from @fig-closure-damage, it compares the overall damage caused by random and targeted closures. Also we see that the targeted closures cause more damage than random closures. As shown in @tbl-closure-damage, targeted closures add 0.49 minutes to reachable orders, while random closures add 0.12 minutes. Targeted closures also make 187 orders unreachable, compared with about 42 orders under random closures.

| Scenario | Roads closed | Reachable orders (%) | Unreachable orders | Mean added time (min) |
|:---------|-------------:|---------------------:|-------------------:|----------------------:|
| Random   | 163          | 99.73                | 41.5               | 0.115                 |
| Targeted | 163          | 98.77                | 187.0              | 0.489                 |

: Network damage under random and targeted road closures. Random results are averaged across 30 trials. {#tbl-closure-damage}

Based on these results, targeted closures create about 4.25 times more delay and 4.5 times more route failures. This shows that closing the most-used roads is much more damaging than closing the same number of random roads.

![Percentage increase in average delivery time.](outputs/18_closure_effect_strategy.png){#fig-closure-strategy width=85%}

As we can see from the @fig-closure-strategy, it shows the percentage increase in delivery time for each assignment strategy and fleet size. The closure effect is almost the same for NAC and SWB. With 200 couriers, targeted closures increase average delivery time by 0.36% under NAC and 0.38% under SWB. With 500 couriers, the increases are 2.26% and 2.20%. The percentages are higher for 500 couriers because their original delivery times are much lower.

To sum up, targeted closures cause much more damage than random closures. However, the assignment strategy does not protect the pickup-to-door route from a closure. This experiment only measures the route from the restaurant to the customer because courier locations before pickup were not available.

## RQ8: Fleet Size

![Average delivery time and percentage of orders.](outputs/19_fleet_delivery_performance.png){#fig-fleet-performance width=85%}

As we can see from @fig-fleet-performance, it shows that the delivery performance improves quickly as fleet size increases. Also, we see that with 100 couriers, average delivery time is 576 minutes, and only 11.75% of orders are delivered within 45 minutes. At 400 couriers, average delivery time falls to 29.81 minutes, but the on-time rate is still only 81.50%. At 500 couriers, average delivery time falls to 21.90 minutes, and both strategies deliver 100% of orders within 45 minutes.

@tbl-ideal-fleet shows the performance of each strategy at 500 couriers.

| Strategy | Mean time (min) | P95 time (min) | Within 45 min (%) | 7 PM mean (min) | Utilization (%) |
|:---------|----------------:|---------------:|------------------:|----------------:|----------------:|
| NAC      | 21.59           | 30.43          | 100.00            | 23.85           | 36.30           |
| SWB      | 22.21           | 31.13          | 100.00            | 24.81           | 32.30           |

: Delivery performance at the ideal fleet size of 500 couriers. {#tbl-ideal-fleet}

NAC is slightly faster than SWB at this fleet size. However, both strategies provide reliable performance throughout the day and during the 7:00 PM demand peak.

![Average minutes saved per 100 additional couriers.](outputs/20_fleet_marginal_benefit.png){#fig-fleet-benefit width=85%}

As we can see from @fig-fleet-benefit, it shows that adding more couriers provides very little improvement after 500. Also, increasing the fleet from 400 to 500 saves 7.91 minutes. However, increasing it from 500 to 600 saves only 0.91 minutes. Increasing it from 600 to 800 saves only another 0.19 minutes. Average utilization also falls from 34.30% at 500 couriers to 23.75% at 800 couriers.

To sum up, the ideal tested fleet size is 500 couriers. It delivers every order within 45 minutes under both strategies, while adding more couriers provides very little improvement. However, this is only a performance-based result because courier wages and other operating costs were not included.

# Limitation

This project has several limitations. First, the results are based on a delivery system in Washington, DC. Therefore, the findings may not fully represent other cities or countries with different delivery systems. Additionally, the population density was used as a proxy for customer demand because real delivery records were not available. Restaurant pickups were also simulated instead of coming from actual order data.

Moreover, the road network uses free-flow travel time and does not include real traffic congestion. Since only about one-third of the roads had speed information, the remaining speeds were estimated by road type. The road-closure experiment only measures the route from the restaurant to the customer because courier locations before pickup were unavailable.

Lastly, we only compared NAC and SWB. Other assignment strategies may produce different results. The ideal fleet of 500 couriers is also based only on delivery performance. Courier wages, fuel costs, and other operating costs were not included. Therefore, 500 couriers should be treated as a performance-based result rather than a financial optimum.

# Conclusion

## Network Structure

To sum up, for the network structure, travel-time weighted betweenness was more useful than traffic volume or speed for finding bottlenecks. Also, we found that the 3rd Street Tunnel, Southeast Freeway, and Florida Avenue are important roads. The demand ceiling for 500 couriers is about 1,461 orders per hour, which is lower than the dinner peak of 2,279 orders. Also, upgrading important roads such as Florida Avenue and New Jersey Avenue performed better than random upgrades, but the improvement was only 0.09%. Lastly, we also found that restaurant clusters helped delivery because clustered restaurants reached demand areas about 16% faster than isolated restaurants.

## Delivery Operations

To sum up, for delivery operations, SWB had the fastest overall average delivery time. However, SWB performed better with 200 couriers, while NAC was slightly faster with 500 couriers. Also, high demand caused serious delays when only 200 couriers were available. Moreover, a fleet of 500 kept average delivery time between 21 and 24 minutes during lunch and dinner. Targeted closures caused about 4.25 times more delay and 4.5 times more route failures than random closures, while NAC and SWB were affected almost equally. Lastly, 500 couriers was the ideal tested fleet size because both strategies delivered every order within 45 minutes, while adding more couriers provided little improvement.

Overall, based on the results,  we conclude that reliable delivery performance depends on both the road network and operational decisions. Delivery platforms need enough couriers to handle peak demand, while city planners should focus on protecting the most important roads.

# AI Usage

ChatGPT was used to help debug code, summarize notebook outputs, organize paper sections, and improve grammar [@openai2026]. The original analysis, code, calculations, figures, and interpretations were completed by the authors. All AI-assisted content was reviewed and verified by the authors.
