...
date: "2026-08-05"
abstract: |
  Urban food delivery depends on two things at once, an assignment policy that pairs orders with couriers and a road network that constrains what any policy can achieve. Most prior work optimizes the assignment side and treats the network as given. We study both sides together for Washington, DC, using the OpenStreetMap drivable network, 2,043 restaurant POIs, census tract population as a demand proxy, and observed 2024 traffic volumes. On the structural side, travel time weighted edge betweenness identifies a short list of chokepoints, led by the 3rd Street Tunnel and the Southeast Freeway, that simple metrics largely miss. Observed volume ranks edges only moderately like betweenness (Spearman 0.42) and the two measures share just 8 percent of their top one percent. Restaurants cluster strongly (Clark-Evans R of 0.32) and clustered restaurants reach demand about 16 percent faster than isolated ones. The network is robust to random closures but roughly forty times more sensitive to targeted ones. On the operational side, a discrete event simulation of 15,188 synthetic orders shows that windowed system-wide matching beats greedy dispatch by about 2 percent under courier scarcity, that a fleet near 500 couriers meets a 45 minute p95 standard against a 20.5 minute physical floor, and that capacity binds at the dinner peak near 1,500 orders per hour rather than at daily volume. Together the results suggest that the topology of a delivery network sets the ceiling that assignment policy can only approach.
format:
  pdf:
...
Bottlenecks are measured with travel time weighted edge betweenness centrality approximated over 150 sampled sources, compared against observed AADT and edge speed as simple alternatives. Renovation and closure effects are measured with small counterfactuals on a common origin destination sample of 320 restaurant to demand pairs, upgrading the highest betweenness slow surface edges (seven under the current threshold) or removing the ten highest betweenness edges, each against random controls of three trials.

## Discrete event simulation

The operational questions use a discrete event simulation over the synthetic day, with the all pairs shortest path travel time matrix precomputed in memory so that every routing query is an array lookup. Orders become ready ten minutes after placement following the ready time treatment of @reyes2018, and dropoffs take two minutes. Two assignment strategies share one commitment rule and differ only in scope. Nearest Available Courier dispatches each order at arrival to the courier who can reach the restaurant soonest. System-wide Batching accumulates orders for 120 second windows and matches the full fleet to waiting orders with a greedy cheapest pair heuristic, a computationally light stand in for the exact matching of @joshi2022 and @liang2024. Five fleet sizes between 100 and 800 couriers were swept in parallel. Couriers do not reposition while idle and orders are not bundled, both simplifications that @cheng2025 and @liang2024 respectively show can be relaxed with profit.
...
As we can see from the plot above, both strategies trace the same capacity curve, collapsing at 100 to 200 couriers and flattening against a physical floor of about 20.5 minutes beyond 500. The smallest fleet meeting a 45 minute p95 standard is 500 under either strategy, which is the practical answer to RQ8, and pushing to 800 buys under a minute while dropping utilization from 36 to 27 percent. The strategy comparison answers RQ5 conditionally. System-wide Batching outperforms greedy dispatch by about 2 percent in mean delivery time under scarcity, for example 135 against 138 minutes at fleet 200, because coordinated matching cuts wasted pickup travel when couriers are the binding resource. The margin is smaller than exact matching would deliver, which suggests part of the batching advantage lives in the optimality of the matching rather than in the batching alone. At abundance the ranking flips by roughly one dispatch window of latency. Which method is fastest therefore depends on the operating regime, and under the regimes that matter, scarcity and peaks, batching wins.

## RQ6: peak load

![Hourly mean click-to-door time under the lunch and dinner peaks.](outputs/13_peak_load_performance.png){#fig-peak width=95%}

As we can see from the plot above, peak performance is not a scaled version of off peak performance but a queueing regime change. At fleet 200 the lunch rush pushes hourly means past two hours and the dinner rush far higher, with each backlog taking hours to drain. At fleet 500 the identical demand passes through with a ripple, a 19:00 mean near 24 minutes against 21 off peak. The dinner peak binds first, so fleet and strategy decisions should be evaluated at 19:00 rather than on daily averages.
...
The structural and operational findings are two views of one mechanism. The street network funnels shortest paths through a handful of crossings, concentrates restaurants in corridors that happen to sit at the point of best reach, and offers enough grid redundancy that only targeted disruption hurts. Those same properties reappear operationally as a hard physical floor of about 20.5 minutes per delivery, a fleet requirement near 500 couriers for acceptable daily service, and a dinner hour rate ceiling that no assignment cleverness can lift, only more couriers or a faster network. Assignment policy matters exactly where the structure leaves room, under scarcity, where windowed system-wide matching recovers about 2 percent of delivery time that greedy dispatch wastes.

Four limitations qualify these results. Travel times are free flow values built largely from imputed speeds, so congestion, which would deepen the peaks and sharpen the bottlenecks, is absent. Demand is a population density proxy rather than observed orders. The batched strategy uses a greedy cheapest pair heuristic rather than exact matching, so the measured batching advantage is a lower bound on what coordinated assignment can deliver. And the simulation excludes order bundling and idle repositioning, both of which the literature shows would improve the batching strategy further [@liang2024, @cheng2025]. Each limitation points the same direction, toward time varying travel times and richer courier behavior on the substrate assembled here, which we leave as the natural next stage of this work.
